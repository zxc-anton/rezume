...
def get_vacancies(offset=0, admin = False):
    """Получение классов резюме"""
    if admin:
        rezume_count = db.session.query(rezume).where(rezume.status == 'no_checked').count()
        result = db.session.query(rezume).where(rezume.status == 'no_checked').offset(offset).limit(10).all()
    else:
        rezume_count = db.session.query(rezume).count()
...
def index():
    ...
    try:
        ...
        if count == 0:
            render_template('index.html', posts=posts, pagins=count, admin = True) 
        return render_template('index.html', posts=posts, pagins=round(count/10), admin = True)
...
def show_rezume():
    ...
    try:
        rezume_id = int(request.args.get('id'))
...
def aprove_rezume():
    ...
    if rezume_id and rezume_id.isdigit():
        query = db.session.query(rezume).filter_by(rezume.id == rezume_id)
